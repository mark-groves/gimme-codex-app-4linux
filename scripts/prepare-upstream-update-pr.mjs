...
const blockedPath = path.resolve(options.blocked);
const channel = options.channel;

const snapshot = await readJson(snapshotPath);
const current = await readJson(currentPath);
const blocked = await readBlockedVersions(blockedPath);
...
const blockedVersion = findBlockedVersion(blocked, channel, actual);
if (blockedVersion) {
  const reason = blockedVersion.reason ? `: ${blockedVersion.reason}` : "";
  await writeGithubOutputs({
    drift: "false",
    blocked: "true",
    channel,
    ...
    summary: `${channel} appcast ${actual.version} (${actual.build}) is blocked${reason}`,
  });
  console.log(`${channel} appcast ${actual.version} (${actual.build}) is blocked${reason}`);
  process.exit(0);
}
...
function parseArgs(argv) {
  const parsed = {
    blocked: "data/blocked-upstream.json",
    channel: "prod",
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--snapshot" || arg === "--current" || arg === "--body" || arg === "--channel" || arg === "--blocked") {
...
async function readBlockedVersions(filePath) {
  try {
    return JSON.parse(await fs.readFile(filePath, "utf8"));
  } catch (error) {
...
function findBlockedVersion(blocked, channel, latest) {
  const blockedChannel = blocked[channel];
  if (!Array.isArray(blockedChannel)) {
    return null;
  }
  return blockedChannel.find((entry) => entry.version === latest.version && entry.build === latest.build) ?? null;
}
