...
On 2026-05-15, the prod `26.513.20950` / build `2816` update from PR #21 was reverted because the converted Linux build could not rebuild its native modules against Electron `42.0.1`. Electron's runtime can still be installed by invoking the package `install.js`, but `better-sqlite3` `12.9.0` and `12.10.0` fail against Electron 42's V8 headers; keep prod on the last buildable `26.506.31421` / build `2620` snapshot until upstream native-module support or a local compatibility patch is available. Local build/update commands now consume the tracked snapshot by default, `--source live` is the explicit live-appcast path, and `data/blocked-upstream.json` prevents the scheduled update automation from recreating PRs for known-unbuildable appcast versions.
