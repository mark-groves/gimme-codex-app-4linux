...
On 2026-05-10, the Linux conversion added build metadata by patching the Electron bootstrap rather than the minified renderer bundle. The metadata injection runs after `app.whenReady()`, attaches to normal app BrowserWindows through `browser-window-created`, skips always-on-top overlay windows, and sets the window title plus renderer metadata to values such as `Codex Linux prod build 2620 from app 26.506.31421`. CodeQL flagged the generated browser script as code construction, so the builder now validates metadata against narrow version, channel, and build-token formats before generating the script; the renderer still assigns values with DOM properties, not HTML parsing. Visual checks found both bottom-right and top-toolbar badge placements competed with real app content/chrome, so the in-window badge is now opt-in with `CODEX_LINUX_VERSION_BADGE=1` instead of being shown by default.
