...
- It injects a small Linux-only version badge into normal app windows after `app.whenReady()` so the launched UI visibly reports the converted app version, channel, and appcast build.
